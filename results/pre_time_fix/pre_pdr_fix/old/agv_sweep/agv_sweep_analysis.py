"""
agv_sweep_analysis.py
=====================
AGV fleet-size sweep analysis for the DFJSP-AGV simulation.

Answers two primary questions:
  1. At what fleet size does makespan start to degrade due to congestion?
  2. What is the recommended AGV count for each Brandimarte instance?

Expected directory layout:
    <sweep-dir>/
      agv5/
        merged_results.csv
        merged_machine_utilization.csv
        merged_agv_performance.csv
        merged_segment_congestion.csv
      agv10/ ...  agv15/ ...  agv20/ ...  agv25/ ...  agv30/ ...

Usage:
    python agv_sweep_analysis.py --sweep-dir agv_sweep/ [--out figs_agv_sweep/]
    python agv_sweep_analysis.py --sweep-dir agv_sweep/ --instances MK01 MK04 MK08
...
MK_ORDER = [f"MK{i+1:02d}" for i in range(15)]


def _clean_instance(val: str) -> str:
    """
    Normalise any variant of a Brandimarte label to MK##.
    Handles: brandimarte_mk04, mk4, MK_04, etc.
    Returns the original string unchanged if no match found.
    """
    m = re.search(r'[Mm][Kk]\s*[_]*(\d+)', str(val))
    return f"MK{int(m.group(1)):02d}" if m else str(val)


def _instance_order(vals) -> list[str]:
    """Return instances in canonical MK01..MK15 order, with unknowns appended."""
    present = set(pd.Series(vals).dropna().unique())
    ordered  = [m for m in MK_ORDER if m in present]
...
def load_sweep(sweep_dir: str | Path) -> dict[str, Optional[pd.DataFrame]]:
    """
    Discover agvN subdirectories under sweep_dir, load all four CSV types,
    tag each row with agv_count (from the folder name), and return a dict
    keyed by domain name.

    Also sanity-checks that the agvCount column in results.csv matches the
    folder-derived agv_count value, warning on any discrepancy.
    """
    sweep_dir = Path(sweep_dir)
    buckets: dict[str, list[pd.DataFrame]] = {k: [] for k in CSV_NAMES}

    agv_dirs = sorted(
        [d for d in sweep_dir.iterdir() if d.is_dir() and re.match(r"^agv\d+$", d.name)],
        key=lambda d: int(re.search(r"\d+", d.name).group()),
    )
    if not agv_dirs:
        raise FileNotFoundError(f"No agvN subdirectories found in {sweep_dir}. "
                                 "Expected folders like agv5/, agv10/, ...")

    print(f"Discovered AGV sweep folders: {[d.name for d in agv_dirs]}")

    for d in agv_dirs:
        agv_n = int(re.search(r"\d+", d.name).group())
        ...
        for key, fname in CSV_NAMES.items():
            ...
            df["agv_count"] = agv_n
            df["instance"]  = df["instance"].astype(str).apply(_clean_instance)
            ...
            if key == "results" and "agvCount" in df.columns:
                ...
                if not mismatch.empty:
                    ...
                    print(f"  [warn] {d.name}: agvCount column has values "
                          f"{found_vals} but folder says {agv_n}. "
    ...
    for key, frames in buckets.items():
        if frames:
            ...
            print(f"  Loaded '{key}': {len(result[key]):,} rows "
                  f"across {len(frames)} AGV levels")
        else:
            result[key] = None
...
#  Secondary y-axis shows congestion fraction.
#  Minimum and elbow markers annotated.
# ══════════════════════════════════════════════════════════════════════════════
...
def fig01_makespan_vs_agv(
    ...
    """
    One subplot per instance.
    Mean makespan ± 1 std across (rule × seed), with congestion fraction on a
    secondary axis.  Green dashed = fleet size at minimum makespan.
    """
    ...
    for idx, inst in enumerate(instances):
        ...
        # Elbow marker (only if different from minimum)
...
# ══════════════════════════════════════════════════════════════════════════════
#  Fig 02 — Per-PDR makespan curves (subset of instances)
# ══════════════════════════════════════════════════════════════════════════════
...
def fig02_makespan_per_pdr(
    ...
    """
    For each instance subplot: one line per PDR, x = agv_count, y = seed-averaged makespan.
    Reveals whether the optimal fleet size is PDR-dependent and which rules
    degrade earliest under congestion.
    """
    instances = _instance_order(results["instance"])
    if instance_filter:
        instances = [i for i in instances if i in instance_filter]
...
def fig03_agv_time_budget(agv: pd.DataFrame, out: Path) -> None:
    """
    Left:  stacked bar — fraction of mean AGV episode time in each state at each fleet size.
    Right: absolute waiting vs idle time per AGV.

    The diagnostic signature of congestion onset: time_idle falls while
    time_waiting_route rises sharply.
    """
    time_cols = [
...
def fig04_congestion_vs_agv(agv: pd.DataFrame, out: Path) -> list[dict]:
    """
    Line per instance showing mean congestion_fraction vs fleet size.
    Vertical dotted lines mark each instance's first CONGESTION_WARN crossing.
    Returns a list of onset records for use in the recommendation table.
    """
...
def fig05_machine_util_vs_agv(machine: pd.DataFrame, out: Path) -> None:
    """
    Left:  mean machine utilization rate per instance vs fleet size.
    Right: fraction of machines starved (<5% util) per instance vs fleet size.

    Plateauing utilization despite adding AGVs indicates the floor is already
    transport-saturated rather than transport-starved.
    """
...
def fig06_segment_heatmap(seg: pd.DataFrame, out: Path) -> None:
    """
    Heatmap: x = AGV count, y = zone (sorted by max block rate), colour = block rate.
    Separate sub-panels per aisle_type.

    Identifies which layout zones congest first and saturate earliest as the
    fleet grows — directly informs AGV routing and layout recommendations.
    """
    seg["block_rate"] = pd.to_numeric(seg["block_rate"], errors="coerce")
    aisle_types = sorted(seg["aisle_type"].dropna().unique())

    if not aisle_types:
        # Try without filtering by aisle type
        aisle_types = ["all"]
        seg = seg.copy()
        seg["aisle_type"] = "all"
    ...
    for ax, atype in zip(axes[0], aisle_types):
        ...
        # Sort zones by peak block rate (worst at top for readability)
        pivot = pivot.loc[pivot.max(axis=1).sort_values(ascending=False).index]

        # Cap height so the figure doesn't explode with many zones
...
def fig07_reroutes_vs_agv(agv: pd.DataFrame, out: Path) -> None:
    """
    Mean reroute count per AGV vs fleet size.

    Reroutes begin to rise before congestion_fraction hits the 20% threshold,
    making this an early-warning signal of path contention.
    """
...
def fig08_congestion_makespan_scatter(
    ...
    """
    Scatter: x = mean congestion fraction, y = mean makespan.
    Each point = one (instance, agv_count) combination.
    Colour encodes fleet size.  Thin grey line connects fleet-size steps per instance.

    Directly visualises the congestion → makespan inflation relationship.
    """
    ms   = (results.groupby(["instance", "agv_count"])["makespan"]
            .mean().reset_index()
    ...
    for inst in _instance_order(merged["instance"]):
        ...
        # Scatter points coloured by fleet size
        sc = ax.scatter(
        ...
        # Label at highest-congestion point for readability
        last = sub.iloc[-1]
...
def fig09_pdr_rank_stability(results: pd.DataFrame, out: Path) -> None:
    """
    For each fleet size, rank PDRs by mean makespan (averaged across instances).
    Plot mean rank vs fleet size per PDR.

    A PDR whose rank worsens at high fleet sizes is congestion-sensitive:
    it was relying on transport slack that disappears as the floor fills up.
    """
...
def fig10_summary_panel(
    ...
    """
    Compact 4-panel overview for thesis/advisor discussions:
      TL: normalised makespan (each instance ÷ its agv5 baseline)
      TR: mean congestion fraction
      BL: mean machine utilization
      BR: PDR spread (CV of makespan across rules) — does fleet size homogenise performance?
    """
...
def make_recommendation_table(
    ...
    Per instance, compute:
      agv_at_min_makespan     fleet size achieving the lowest mean makespan
      elbow_agv               Kneedle-lite elbow of the makespan curve
      makespan_improvement%   % gain from the smallest fleet tested to the optimum
      congestion_onset_agv    first fleet size where mean congestion_fraction >= WARN
      recommended_agv         min-makespan fleet, capped at congestion onset
      regime                  monotone-decreasing / monotone-increasing / has-minimum
    """
    ms_g = (results.groupby(["instance", "agv_count"])["makespan"]
            .mean().reset_index())
    ...
    for inst in _instance_order(ms_g["instance"]):
        ...
        # Recommended: minimum makespan, but no higher than congestion onset
...
def main() -> None:
    global CONGESTION_WARN, CONGESTION_ALARM  # may be overridden via CLI args

    p = argparse.ArgumentParser(
        description="AGV fleet-size sweep analysis for DFJSP-AGV simulation results.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python agv_sweep_analysis.py --sweep-dir Results/agv_sweep/
  python agv_sweep_analysis.py --sweep-dir Results/agv_sweep/ --out figs/agv_sweep/
  python agv_sweep_analysis.py --sweep-dir Results/agv_sweep/ --instances MK01 MK04 MK08
        """,
    )
    p.add_argument(
        "--sweep-dir", required=True, metavar="DIR",
        help="Root directory containing agv5/, agv10/, agv15/, ... subdirectories.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--instances", nargs="+", metavar="MK##",
        help="Restrict per-instance detail plots to these instances, e.g. MK01 MK04 MK08. "
    ...
    # Normalise any --instances values passed with lowercase or underscores
    instance_filter = None
    if args.instances:
        instance_filter = [_clean_instance(i) for i in args.instances]
    ...
    # Warn if any (instance, agv_count, rule) combination is missing seeds
